...
public class ImageGuesser {
    ...
    private Font pixelFont = GameShowUI.pixelFont();

    private final static String submitButtonPath = "./GameShowImages/submit-button.png";
    private static final ImageIcon submitButtonIcon = new ImageIcon(submitButtonPath);

    private JPanel outcomeContainer;
    private static final Logger logger = Logger.getLogger(ImageGuesser.class.getName());

    public ImageGuesser(MinigameNetworkClient mnClient, GameMetadata game, JPanel gameContainer, String player,
            String imageFileName, int gameId) {
    ...
    public void startImageGuesser() {
        ...
        JButton submitButton = new JButton(
                new ImageIcon(submitButtonIcon.getImage().getScaledInstance(300, 130, Image.SCALE_DEFAULT)));
    ...
    public void sendCommand(JsonObject json) {
        ...
        this.mnClient.send(new CommandPackage(this.game.gameServer(), this.game.name(), this.player,
